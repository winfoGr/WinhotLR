...
Public Class ErgasiesHmerasF
    ...
    Private Sub GrammPelatis(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GrammPelatisTbx.Enter
        ...
        Dim form As New KratiseisSucheF(imeromErgasias, True, False, 3)
    ...
    Private Function ekdosi_ge_transaction(ByVal connectionstring As String) As Int16
        ...
        Using connection As New OleDb.OleDbConnection(connectionstring)
            ...
            Try
                ...
                command.CommandText = "INSERT INTO gramatia (aa, imerominia, diamenwn, poso, akyro)" & _
    ...
    Private Sub suche_mem_apy_diamenontes(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not MemApyDwmTbx.Text.Equals("") Then
            ...
            If Me.DbhotelDataSet.kratiseis.Count = 1 Then '
                ...
                If kwdApyAfm <> -1 AndAlso kwdApyAfm <> get_etairia_ekdosis_tim(Me.DbhotelDataSet.kratiseis(0).dwmatio) Then
    ...
    Private Sub MemApyEkdBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MemApyEkdBtn.Click
        ...
        erzeuge_apy(True, ist_mauri_etaireia(kwdApyAfm))
    ...
    Private Sub MemApySynexBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MemApySynexBtn.Click
        ...
        End If
        If kwdApyApy <> -1 Then
            response = MsgBox("Δεν υφίσταται αυτή η επιλογή για εταιρείες !" & vbCrLf & "Εκδοση του Παραστατικού; ", MsgBoxStyle.YesNo, "winfo\nikEl.")
            ...
            Exit Sub
        End If
        erzeuge_apy(False, ist_mauri_etaireia(kwdApyAfm))
    ...
    Private Function ist_mauri_etaireia(ByVal kwd As Integer)
        ...
        Return istEikoniki
    End Function
    Private Sub erzeuge_apy(ByVal ekdosiOk As Boolean, ByVal mauro As Boolean)
        Dim j, rows As Int16
        ...
        If kwdApyKrat <> -1 Then
            ...
            For j = 0 To Me.DbhotelDataSet.timeskratisis.Count - 1
                ...
                geumaNeto, geumaFpa, deipnoNeto, deipnoFpa, extraNeto, extraFpa, 0, 0, sumKratPosa, sumKratFpa, sinola, Me.DbhotelDataSet.timeskratisis(j).xrewsi, Me.DbhotelDataSet.timeskratisis(j).apo, _
        ...
        If ekdosiOk AndAlso Me.DbhotelDataSet.ekt_apy_mem.Count <> 0 Then
            ...
            Using connection As New OleDb.OleDbConnection(connectionString)
                ...
                Try
                    ...
                    'MsgBox(Me.DbhotelDataSet.ekt_apy_diafora(0).dim_foros)
                    ...
                    "afm, doy, sumkathposa, fpa,  synola, prokataboli, simetoxi, ypoloipon, pliromi, dimforos, exoflithi, akyromeno, extra, memonomeno, mauro,identifikation) " & _
                    "VALUES (parastatiko, imerominia, seira, aa, praktoreio, simbolaio, eponimia, dieuthinsi, afm, doy, sumkathposa, fpa,  synola, prokataboli, simetoxi, ypoloipon," & _
                    "pliromi, dimforos, exoflithi, false, false, true, mauro, identifikation)"
                    command.ExecuteNonQuery()
                    command.Parameters.Clear()

                    ' to IsolationLevel.ReadCommitted den blepei allages pou den exoun ginei commited apo allon User->iparxei pithanotita na exei diabasei 
                    'kapoios allos ton idio aa apo ta parastatika Table->giauto kanw abfrage nochmal ob o allos User exei kanei entwmwtaksi Commited kai
                    'fanei oti to idio aa exei dothei kai allou->(stin extrem periptwsi pou oi User einai entelws suxronismenoi (mazi commited) tote
                    'mporei na iparksoun 2 timologia me ton Idio aa(->iparxei to IsolationLevel.ReadUncommitted pou mporei na dei allages pou den exoun
                    'ginei commited alla thewrw oti einai toso extrem fall pou den to xrisimopoiw)
                    ...
                    For j = 0 To kwdProsEkd.Length - 1
                        ...
                        If rows <> 1 Then
                            ...
                            Exit Sub
                        End If
                        'DEN EXEI NOHMA, DA TWRA MONO APY ANAXWRISIS KAI OXI KATA THN AFIXI
                        'command.Parameters.AddWithValue("arapy", aaApyParast)
                        'command.Parameters.AddWithValue("kratisi", CType(kwdProsEkd(j), Integer))
                        'command.CommandText = "UPDATE biblioportas SET arapy=? where (kratisi=?)"
                        'rows = command.ExecuteNonQuery()
                        'command.Parameters.Clear()
            ...
            '                Me.TimologiaTableAdapter.InsertTimologio(Me.DbhotelDataSet.etaireia(0).aaapy, imeromErgasias, seira, aaApyParast, kwdApyPrakt, Nothing, sumSumFpa, sumSynola, _
            'prokatKrat, MemApyTrPlCbx.SelectedItem, sumSumPosa, False, exoflithi, simetKrat, Me.DbhotelDataSet.ekt_apy_diafora(0).ypoloipon, _
            'Me.DbhotelDataSet.ekt_apy_diafora(0).afm_adt, Me.DbhotelDataSet.ekt_apy_diafora(0).address, Me.DbhotelDataSet.ekt_apy_diafora(0).doy, Nothing, Me.DbhotelDataSet.ekt_apy_diafora(0).eponimia, Me.DbhotelDataSet.ekt_apy_diafora(0).dim_foros, False, True, False, simbKrat, timolID)

            '                Try
            '                    kwdNeouTimol = Me.TimologiaTableAdapter.GetKwdByParastImeromAAID(Me.DbhotelDataSet.etaireia(0).aaapy, imeromErgasias, aaApyParast, timolID)
            '                Catch ex As InvalidOperationException ' kanonika den prepei na sinbei!!!!!!
            '                    MsgBox("Παρουσιάστηκε πρόβλημα στην Εκδοση ΑΠΥ", MsgBoxStyle.Critical, "winfo\nikEl.")
            '                    Exit Sub
            '                End Try
            '                'kanw select ton DB kwdiko->prosoxi ean yparxei concurency(tautoxrona allos user ebale allo timogio me idio aa) 

            '                'elegxos ean uparxei diplokataxwrisi aa me tin idia imerominia 
            '                countTimol = Me.TimologiaTableAdapter.CountTimolByParastImerAA(Me.DbhotelDataSet.etaireia(0).aaapy, imeromErgasias, aaApyParast)

            '                If countTimol = 1 Then 'den uparxei ara o aa eanai swstos
            '                    ' elegxw ean entwmetaksi i kratisi timologithike apo allo user->se periptwsi synexeias kratisis pou mesolabei polu wra mexri na epilegei mia kratisi
            '                    '->se autin thn wra allos User mporei na tin exei IDI timologisei
            '                    If check_kratiseis_ob_nicht_mittlerweile_timolog(kwdApyPrakt) Then
            '                        Me.TimologioanalysisTableAdapter.SetKwdTimolByMinusEinsTimolID(kwdNeouTimol, -1, timolID)
            '                        Me.KratiseisTableAdapter.UpdateTimologioByTimologio(1, 2)
            '                    Else
            '                        Me.TimologioanalysisTableAdapter.RollbackTimAnalysis(-1, timolID)
            '                        Me.TimologiaTableAdapter.RollBackTimologia(timolID, kwdNeouTimol)
            '                        MsgBox("Παρουσιάστηκε πρόβλημα στην Εκδοση ΑΠΥ", MsgBoxStyle.Critical, "winfo\nikEl.")
            '                        Exit Sub
            '                    End If
            '                    Me.ParastatikaTableAdapter.UpdateAAByKwd(aaApyParast, Me.DbhotelDataSet.etaireia(0).aaapy)
            '                    commitOK = True
            '                Else 'uparxei diplokataxwrisi  aa
            '                    If countFehl < 0 Then 'diplokataxwrisi aa me tin idia imerominia->sunolika 5 Versuche
            '                        countFehl += 1
            '                        Me.TimologiaTableAdapter.RollBackTimologia(timolID, kwdNeouTimol) 'delete timologio me timolID ->einai seasion bedingt->
            '                        Randomize()
            '                        ' Generate random value between 1 and 5.
            '                        Dim sekunde As Integer = CInt(Int((5 * Rnd()) + 1))
            '                        Wait(sekunde) 'perimene mexri 5 sekunde_>texniki gia na ksesunxronistoun oi User !!! 
            '                    Else '
            '                        Me.TimologioanalysisTableAdapter.RollbackTimAnalysis(-1, timolID)
            '                        Me.TimologiaTableAdapter.RollBackTimologia(timolID, kwdNeouTimol)
            '                        MsgBox("Παρουσιάστηκε πρόβλημα στην Εκδοση ΑΠΥ", MsgBoxStyle.Critical, "winfo\nikEl.")
            ...
            'ean alakse edw i ekptosi tote enhmerwsi kai twn kratisewn Table
            'If posostoTemp <> MemApyPosostoTbx.Text OrElse posoTemp <> MemApyPosoTbx.Text OrElse freeTemp <> MemApyImerFreeTbx.Text OrElse cfeeTemp <> MemApyFeeTbx.Text Then
            '    If MemApyPosostoTbx.Text = 0 AndAlso MemApyPosoTbx.Text = 0 AndAlso MemApyImerFreeTbx.Text = 0 AndAlso MemApyFeeTbx.Text = 0 Then
            '        ekptosiKratOK = False
            '    Else
            '        ekptosiKratOK = True
            '    End If
            '    Me.KratiseisTableAdapter.UpdateEkptoseisByKwd(MemApyImerFreeTbx.Text, ekptosiKratOK, MemApyPosoTbx.Text, MemApyPosostoTbx.Text, MemApyFeeTbx.Text, sumSynola, kwdApyKrat)

            'End If

            proepiskopisi_apy()
            'MsgBox(Me.DbhotelDataSet.etaireia(0).dieuthinsi)
            ...
            If Me.DbhotelDataSet.epinoikiasi.Count = 1 Then
                Dim promithprakt, posostoTemp, poso As Single
                Dim memo As String
                promithprakt = Me.PraktoreiaTableAdapter.GetPromithDomisiByKwd(kwdApyPrakt)
                posostoTemp = 100 - (Me.DbhotelDataSet.epinoikiasi(0).pososto - promithprakt)
                poso = sumSynola * posostoTemp / 100
                memo = "Βίλα " + apyDwm + " " + diarkeia + " Σύνολο: " + sumSynola.ToString + " Domisi: " + (sumSynola - poso).ToString("N")
                If enimerwsi_maintance(Me.DbhotelDataSet.epinoikiasi(0).kwdikos, poso, memo) Then
    ...
    End Sub
    Private Function enimerwsi_maintance(ByVal kwdikos As Integer, ByVal poso As Single, ByVal memo As String) As Boolean
        Dim istOK As Boolean = True
        Dim connString as String = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + Me.DbhotelDataSet.etaireia(0).dieuthinsi
        Using connection As New OleDb.OleDbConnection(connString)
            ...
            Try
                ...
                command.Parameters.AddWithValue("ΠΟΣΟΝ", poso)
                command.Parameters.AddWithValue("ΗΜΝΙΑ", imeromErgasias)
                command.Parameters.AddWithValue("ΜΕΜΟ", memo)
                command.CommandText = "INSERT INTO ΠΛΗΡΩΜΕΣ (CustomerID,ΠΟΣΟΝ,[ΗΜ/ΝΙΑ], ΜΕΜΟ) VALUES (CustomerID,ΠΟΣΟΝ,ΗΜΝΙΑ,ΜΕΜΟ)"
                command.ExecuteNonQuery()
                command.Parameters.Clear()
            Catch ex As Exception
    ...
    Private Sub EkdosiParastParastCbx_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EkdosiParastParastCbx.SelectedIndexChanged
        ...
        If sender.selectedindex <> -1 Then
            ...
            Try
                ...
                aaParast = Me.DbhotelDataSet.parastatika(0).aritthmos + 1
    ...
    Private Sub EkdosiParastYpnosTbx_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EkdosiParastYpnosTbx.Leave, EkdosiParastPrwinoTbx.Leave, _
    ...
    End Sub
    Private Sub EkdosiParastYpnosFpaTbx_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EkdosiParastYpnosTbx.Leave, EkdosiParastPrwinoTbx.Leave, _
       EkdosiParastGeumaTbx.Leave, EkdosiParastDeipnoTbx.Leave, EkdosiParastAllTbx.Leave, EkdosiParastProkatabTbx.Leave, EkdosiParastYpnosFpaTbx.Leave, EkdosiParastPrwinoFpaTbx.Leave, _
       EkdosiParastGeumaFpaTbx.Leave, EkdosiParastDeipnoFpaTbx.Leave, EkdosiParastAllFpaTbx.Leave, EkdosiParastProkatabTbx.Leave
        ...
        EkdosiParastSynolaTbx.Text = s.ToString("N")
    End Sub
    Private Sub EkdosiParastYpnosTbx_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EkdosiParastYpnosTbx.Click, EkdosiParastPrwinoTbx.Click, _
      EkdosiParastGeumaTbx.Click, EkdosiParastDeipnoTbx.Click, EkdosiParastAllTbx.Click, EkdosiParastProkatabTbx.Click, EkdosiParastYpnosFpaTbx.Click, EkdosiParastPrwinoFpaTbx.Click, _
       EkdosiParastGeumaFpaTbx.Click, EkdosiParastDeipnoFpaTbx.Click, EkdosiParastAllFpaTbx.Click, EkdosiParastProkatabTbx.Click

        sender.SelectAll()
    End Sub
    ' Private Sub EkdosiParastYpnosTbx_Keypress(ByVal sender As System.Object, ByVal e As KeyPressEventArgs) Handles EkdosiParastYpnosTbx.KeyPress, EkdosiParastPrwinoTbx.KeyPress, _
    'EkdosiParastGeumaTbx.KeyPress, EkdosiParastDeipnoTbx.KeyPress, EkdosiParastAllTbx.KeyPress, EkdosiParastProkatabTbx.KeyPress, EkdosiParastYpnosFpaTbx.KeyPress, EkdosiParastPrwinoFpaTbx.KeyPress, _
    ...
    Private Sub EkdosiParastKataxBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EkdosiParastKataxBtn.Click
        Try
            If ekdosi_extra_parastatikou() Then
                init_parast_felder(True)
                MsgBox(" Επιτυχής καταχώριση ! ", MsgBoxStyle.Information, "winfo\nikEl.")
            End If
        Catch ex As Exception
    ...
    Private Sub PinakasDwmBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PinakasDwmBtn.Click
        ...
        Try
            ...
            For j = 0 To Me.DbhotelDataSet.dwmatia.Count - 1
                ...
                dwmatia(j) = Me.DbhotelDataSet.dwmatia(j).arithmos
            Next
            ...
            If (PinakasDwm2Rdb.Checked) Then 'kai ATSEKARISTES AFIXEIS  mazi
                ...
                Me.KatastasiapyTableAdapter.FillKatastasiProistamenismekaiAfix(Me.DbhotelDataSet.katastasiapy, PinakasDwmPck.Value.Date, PinakasDwmPck.Value.Date, zimmerApo, zimmerEws, "Ov-Book")
            Else 'mono osoi exoun kanei Checkin!
                ...
                Me.KatastasiapyTableAdapter.FillKatastasiProistamenisChkIn(Me.DbhotelDataSet.katastasiapy, PinakasDwmPck.Value.Date, PinakasDwmPck.Value.Date, True, zimmerApo, zimmerEws, "Ov-Book")
            End If
            ...
            For j = 0 To dwmatia.Length - 1
                ...
                index = get_dwmatio_dataset_index(dwmatia(j))
                cot = ""
                If index.Length <> 0 Then 'length=0 ->dwmatio Keno, length=1 Dwmatio 1mal vergeben, length>1 dwmatio vielmal vergeben!
                    ...
                    For i = 0 To index.Length - 1
                        If index.Length > 1 Then 'doppel Vergeben ->diarkeia apo thn pio PROSFATH kratisi
                            indiz1 = max_afixi(index)
                            indiz2 = max_anaxwrisi(index)
                            diarkiaDiam = afix_anax(Me.DbhotelDataSet.katastasiapy(indiz1).afixi.Date, Me.DbhotelDataSet.katastasiapy(indiz2).anaxwrisi.Date)
                        Else
                            indiz1 = index(i)
                            diarkiaDiam = afix_anax(Me.DbhotelDataSet.katastasiapy(indiz1).afixi.Date, Me.DbhotelDataSet.katastasiapy(indiz1).anaxwrisi)
                        End If

                        enilikes = Me.DbhotelDataSet.katastasiapy(index(i)).enilikes

                        paidia = Me.DbhotelDataSet.katastasiapy(index(i)).paidia
                        xrewsi = Me.DbhotelDataSet.katastasiapy(index(i)).xrewsi
                        Try
                            cot = Me.DbhotelDataSet.katastasiapy(index(i)).cot
                        Catch ex As Exception
                        ...
                        imerPets = Me.DbhotelDataSet.katastasiapy(index(i)).petsetes
                        If imerPets = 0 Then
                            imerPets = 15
                        End If
                        imerSent = Me.DbhotelDataSet.katastasiapy(index(i)).sentonia
                        If imerSent = 0 Then
                            imerSent = 15
                        End If

                        'twra kanw plazieren sto DATSET tis ektiposi (counter=4-> 4 dwmmatia ana seira ektiposis sto CReport)
                        If Me.DbhotelDataSet.katastasiapy(index(i)).afixi.Date = PinakasDwmPck.Value.Date Then
                            If counter = 1 Then
                            ...
                            ElseIf counter = 4 Then
                                ...
                                End If
                            End If
                        ElseIf Me.DbhotelDataSet.katastasiapy(index(i)).anaxwrisi.Date = PinakasDwmPck.Value.Date Then 'ANXWRISI
                        ...
                        Else 'ENDIAMESA
                            If counter = 1 Then
                                ...
                                '    Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).sentonia1 = "Σ"
                                'End If
                                If ist_petsetes_sentonia(imerPets, PinakasDwmPck.Value.Date, Me.DbhotelDataSet.katastasiapy(indiz1).afixi.Date) Then
                                    Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).petsetes1 = "Π"
                                End If
                                If ist_petsetes_sentonia(imerSent, PinakasDwmPck.Value.Date, Me.DbhotelDataSet.katastasiapy(indiz1).afixi.Date) Then
                                    Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).sentonia1 = "Σ"
                                End If
                                Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).imerominies1 = diarkiaDiam
                                If Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).doppel1 Is Nothing Then
                            ...
                            ElseIf counter = 2 Then
                                ...
                                Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).paidiaIn2 = Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).paidiaIn2 + paidia
                                Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).cot2 = Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).cot2 + cot
                                If ist_petsetes_sentonia(imerPets, PinakasDwmPck.Value.Date, Me.DbhotelDataSet.katastasiapy(index(i)).afixi.Date) Then
                                    Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).petsetes2 = "Π"
                                End If
                                If ist_petsetes_sentonia(imerSent, PinakasDwmPck.Value.Date, Me.DbhotelDataSet.katastasiapy(index(i)).afixi.Date) Then
                                    Me.DbhotelDataSet.ekt_pinakas_dwmatiwn(Me.DbhotelDataSet.ekt_pinakas_dwmatiwn.Count - 1).sentonia2 = "Σ"
                                End If
    ...
    Private Sub normalize_katastasi()
        ...
        Do Until startindex > Me.DbhotelDataSet.katastasiapy.Count - 1
            countIndex = 1 'posa eintraege 

            If startindex < Me.DbhotelDataSet.katastasiapy.Count - 1 Then
                'i katastasi apy einai Sortiert kaata kwd kratisis!
                While Me.DbhotelDataSet.katastasiapy(startindex).kwd = Me.DbhotelDataSet.katastasiapy(startindex + countIndex).kwd
                    countIndex += 1
                    If startindex + countIndex > Me.DbhotelDataSet.katastasiapy.Count - 1 Then
                        Exit While
                    End If
                End While
                ...
                If countIndex <> 1 Then 'ena MONO ena tupel (diladi hoechstens 1 Paidi stin Kratisi)->nichts tun!!!
                    remake_dataset(startindex, countIndex)
                End If
    ...
    Private Sub katast_estiatoriou()
        ...
        EtikLbl.Text = "ΚΑΤΑΣΤΑΣΗ ΕΣΤΙΑΤΟΡΙΟΥ"
        EtiketaErgHmPnl.Controls.Add(EtikLbl)
        EtiketaErgHmPnl.Location = New Point(5, 5)
        EtiketaErgHmPnl.Size = New Point(ZentralPnl.Width - 20, 60)
        ZentralPnl.Controls.Add(EtiketaErgHmPnl)
    ...
    Private Sub EstiatKatastDwmApoTbx_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EstiatKatastDwmApoTbx.Leave, EstiatKatastDwmMexriTbx.Leave
    ...
    Private Sub EstiatKatastProepBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EstiatKatastProepBtn.Click
        ...
        Try
            ...
            If (EstiatKatast2Rdb.Checked) Then 'kai afixeis mazi
                'Me.KratiseisTableAdapter.FillByAfAnaxCheckInOutDwm(Me.DbhotelDataSet.kratiseis, EstiatKatastPck.Value.Date, EstiatKatastPck.Value.Date, False, False, zimmerApo, zimmerEws)
                Me.KatastasiapyTableAdapter.FillkatastasiEstiatorioumeAfixeis(Me.DbhotelDataSet.katastasiapy, EstiatKatastPck.Value.Date, EstiatKatastPck.Value.Date, EstiatKatastPck.Value.Date, zimmerApo, zimmerEws, "Ov-Book")
            Else 'mono osoi menoun mesa
                'Me.KratiseisTableAdapter.FillByAfAnaxCheckInOutDwm(Me.DbhotelDataSet.kratiseis, EstiatKatastPck.Value.Date, EstiatKatastPck.Value.Date, True, True, zimmerApo, zimmerEws)
                Me.KatastasiapyTableAdapter.FillKatastasiEstiatoriouIn(Me.DbhotelDataSet.katastasiapy, EstiatKatastPck.Value.Date, EstiatKatastPck.Value.Date, EstiatKatastPck.Value.Date, True, zimmerApo, zimmerEws, "Ov-Book")
            End If
    ...
    Private Sub plirotita_xenodox()
        ...
        EtikLbl.Text = "ΠΛΗΡΟΤΗΤΑ ΞΕΝΟΔΟΧΕΙΟΥ"
        ...
        Try
            ...
            ElseIf klinesKwd > 0 And tiposKwd = 0 Then 'kathorismnes klines anexartitou tipou dwmatiou
                ...
                If Me.DbhotelDataSet.bookingpos.Count <> 0 Then
                    fuehle_datum_book_xenodox(datumBook)
                End If
                Me.KratiseisTableAdapter.FillByKlAfAnax(Me.DbhotelDataSet.kratiseis, klinesKwd, XenodStatusImerPck.Value.Date.AddDays(30), XenodStatusImerPck.Value.Date)
                If Me.DbhotelDataSet.kratiseis.Count <> 0 Then
                    fuehle_datum_krat_xenodox(datumKlDwm)
                End If
            ...
            Else
                ...
                If Me.DbhotelDataSet.bookingpos.Count <> 0 Then
                    fuehle_datum_book_xenodox(datumBook)
                End If
                Me.KratiseisTableAdapter.FillByAfAnax(Me.DbhotelDataSet.kratiseis, XenodStatusImerPck.Value.Date.AddDays(30), XenodStatusImerPck.Value.Date)
                If Me.DbhotelDataSet.kratiseis.Count <> 0 Then
                    fuehle_datum_krat_xenodox(datumKlDwm)
                End If
        ...
        If indexPraktStat <> -1 Then
            Try
                If indexPraktStat <= praktStatus.Length - 1 Then
                    Me.DbhotelDataSet.allotment.Clear()
                    Me.AllotmentTableAdapter.FillBySimbKlTipApMexGuar(Me.DbhotelDataSet.allotment, praktStatus(indexPraktStat).simb, praktStatus(indexPraktStat).klines, _
                   praktStatus(indexPraktStat).tipos, StPraktImerPck.Value.Date.AddDays(30), StPraktImerPck.Value.Date, praktStatus(indexPraktStat).guarantee)
                    ...
                    End If
                    Me.BookingposTableAdapter.FillBySimbKlTipApMexGuar(Me.DbhotelDataSet.bookingpos, praktStatus(indexPraktStat).simb, praktStatus(indexPraktStat).klines, _
                               praktStatus(indexPraktStat).tipos, StPraktImerPck.Value.Date.AddDays(30), StPraktImerPck.Value.Date, praktStatus(indexPraktStat).guarantee)
                    If Me.DbhotelDataSet.bookingpos.Count <> 0 Then
                    ...
                    End If
                    Me.KratiseisTableAdapter.FillBySimbKlTipAfAnGuar(Me.DbhotelDataSet.kratiseis, praktStatus(indexPraktStat).simb, praktStatus(indexPraktStat).klines, _
                                                 praktStatus(indexPraktStat).tipos, StPraktImerPck.Value.Date.AddDays(30), StPraktImerPck.Value.Date, praktStatus(indexPraktStat).guarantee)
                    If Me.DbhotelDataSet.kratiseis.Count <> 0 Then
    ...
    Private Sub ekt_st_prakt()
        ...
        For i = 0 To praktStatus.Length
            ...
            If i <= praktStatus.Length - 1 Then
                Me.DbhotelDataSet.allotment.Clear()
                Me.AllotmentTableAdapter.FillBySimbKlTipApMexGuar(Me.DbhotelDataSet.allotment, praktStatus(i).simb, praktStatus(i).klines, _
               praktStatus(i).tipos, StPraktImerPck.Value.Date.AddDays(30), StPraktImerPck.Value.Date, praktStatus(i).guarantee)
                ...
                If Me.DbhotelDataSet.allotment.Count <> 0 Then
                    ...
                    addiere(datumAllotm, datumAllotmSum)
                End If
                Me.BookingposTableAdapter.FillBySimbKlTipApMexGuar(Me.DbhotelDataSet.bookingpos, praktStatus(i).simb, praktStatus(i).klines, _
                           praktStatus(i).tipos, StPraktImerPck.Value.Date.AddDays(30), StPraktImerPck.Value.Date, praktStatus(i).guarantee)
                If Me.DbhotelDataSet.bookingpos.Count <> 0 Then
                    fuehle_datum_book(datumBook)
                ...
                Me.KratiseisTableAdapter.FillBySimbKlTipAfAnGuar(Me.DbhotelDataSet.kratiseis, praktStatus(i).simb, praktStatus(i).klines, _
                           praktStatus(i).tipos, StPraktImerPck.Value.Date.AddDays(30), StPraktImerPck.Value.Date, praktStatus(i).guarantee)
                If Me.DbhotelDataSet.kratiseis.Count <> 0 Then
                    fuehle_datum_krat(datumKrat)
                ...
                For j = index1 To index2
                    ...
                    While myReader.Read()
                        ...
                        If zeiger1 >= 0 AndAlso zeiger2 >= 0 Then
                            ...
                            setze_status(zeiger1, zeiger2, myReader.GetInt32(5), imeresStat, myReader.GetInt32(2))
                        End If
    ...
    End Sub
    'DOMISI
    Private Sub setze_status(ByVal zeiger1 As Integer, ByVal zeiger2 As Integer, ByVal dwmStatus As Integer, ByRef imeresStat() As Integer, ByVal kwdkrat As Integer)
        Dim i As Integer
    ...
    Private Sub emf_diamenontwn()
        ...
        EtikLbl.Text = "ΕΜΦΑΝΙΣΗ ΔΙΑΜΕΝΟΝΤΩΝ"
        ...
        Try
            ...
            If NonShowRdb.Checked = True Then
                ...
                Else
                    ...
                    Me.NonshowkratiseisTableAdapter.FillByAfixi(Me.DbhotelDataSet.nonshowkratiseis, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date)
                End If
            ...
            ElseIf OverRdb.Checked = True Then
                ...
                Else
                    ...
                    Me.AfixeisAnaxwriseis1TableAdapter.OverByEtosAfAnApoMexri(Me.DbhotelDataSet.AfixeisAnaxwriseis1, etos, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date)
                End If
            ...
            Else
                ...
                If DiamenRdb.Checked = True Then
                    ...
                    ElseIf praktDiamKwd = -1 AndAlso simbDiamKwd = -1 Then
                        ...
                        Me.AfixeisAnaxwriseis2TableAdapter.DiamenontesByEtosImeromDwm_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, dwmApo, dwmEws, True, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, etos, True, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date)
                    End If
                ElseIf EpithimiaRdb.Checked = True Then
                    ...
                    ElseIf praktDiamKwd = -1 AndAlso simbDiamKwd = -1 Then
                        ...
                        Me.AfixeisAnaxwriseis2TableAdapter.DiamenontesEpithimiaByEtosImeromDwm_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, "", dwmApo, dwmEws, True, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, etos, "", True, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date, DiamImeromPck.Value.Date)
    ...
    Private Sub ekt_afix_anax_zeig()
        'simbK = -1
        'praktK = -1
        ...
        'Etiketa panel initialisieren
        EtikLbl.Text = "ΚΑΤΑΣΤΑΣΗ ΑΦΙΞΕΩΝ - ΑΝΑΧΩΡΗΣΕΩΝ"
        EtikLbl.Location = New Point(EtiketaErgHmPnl.Width / 5, 25)
        EtiketaErgHmPnl.Controls.Add(EtikLbl)
        EtiketaErgHmPnl.Location = New Point(5, 5)
        EtiketaErgHmPnl.Size = New Point(ZentralPnl.Width - 20, 60)
        ...
        Try
            ...
            If AfixeisRdb.Checked Then
                ...
                If praktK <> -1 AndAlso simbK <> -1 Then
                    ...
                    Me.AfixeisAnaxwriseis2TableAdapter.AfixeisByEtosImeromPraktSimb_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, KatastPrakt.Text, KatastSimb.SelectedItem.ToString, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, etos, KatastPrakt.Text, KatastSimb.SelectedItem.ToString, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date)
                ElseIf praktK <> -1 AndAlso simbK = -1 Then
                    ...
                    Me.AfixeisAnaxwriseis2TableAdapter.AfixeisByEtosImeromPrakt_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, KatastPrakt.Text, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, etos, KatastPrakt.Text, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date)
                ElseIf praktK = -1 AndAlso simbK = -1 Then
            ...
            ElseIf KratiseisRdb.Checked Then
                ...
                ElseIf praktK = -1 AndAlso simbK = -1 Then
                    'Me.AfixeisAnaxwriseisTableAdapter.AfixeisByEtosImerom(Me.DbhotelDataSet.AfixeisAnaxwriseis, etos, ImerKatastPicker.Value.Date)
                    Me.AfixeisAnaxwriseis2TableAdapter.FillKratiseisByImeromKratisis_ChkinAfix(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, False, ImerKatastPicker.Value.Date)
                End If
            ...
            ElseIf AnaxwriseisRdb.Checked Then
                If praktK <> -1 AndAlso simbK <> -1 Then
                    ...
                    Me.AfixeisAnaxwriseis2TableAdapter.AnaxwriseisByEtosImeromPraktSimb_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, KatastPrakt.Text, KatastSimb.SelectedItem.ToString, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date.AddDays(-1), ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, etos, KatastPrakt.Text, KatastSimb.SelectedItem.ToString, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date.AddDays(-1), ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date)
                ElseIf praktK <> -1 AndAlso simbK = -1 Then
                    ...
                    Me.AfixeisAnaxwriseis2TableAdapter.AnaxwriseisByEtosImeromPrakt_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, KatastPrakt.Text, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date.AddDays(-1), ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, etos, KatastPrakt.Text, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date.AddDays(-1), ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date)
                ElseIf praktK = -1 AndAlso simbK = -1 Then
                    ...
                    Me.AfixeisAnaxwriseis2TableAdapter.AnaxwriseisByEtosImerom_(Me.DbhotelDataSet.AfixeisAnaxwriseis2, etos, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date.AddDays(-1), ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date, etos, ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date.AddDays(-1), ImerKatastPicker.Value.Date, ImerKatastPicker.Value.Date)
                End If
                Me.DbhotelDataSet.ekt_AfixAnax.Clear()
                Me.DbhotelDataSet.ekt_AfixAnax.Rows.Add()
                ...
                If tempKwd <> Me.DbhotelDataSet.AfixeisAnaxwriseis2(j).kwd Then
                    ...
                    If Not kratOK Then
                        Try
                            ...
                            '    Me.DbhotelDataSet.AfixeisAnaxwriseis2(j).statusdwmatio = "Ov-Book"
                            'End If
                            If Not (Me.DbhotelDataSet.AfixeisAnaxwriseis2(j).enarxi <= ImerKatastPicker.Value.Date And _
                                                                                             Me.DbhotelDataSet.AfixeisAnaxwriseis2(j).lixi >= ImerKatastPicker.Value.Date.AddDays(-1)) Then
                                Me.DbhotelDataSet.AfixeisAnaxwriseis2(j).statusdwmatio = "Ov-Book"
                            End If
                        Catch ex As StrongTypingException
        ...
        If kwdKratLDB > 0 Then
            If tmimaLKwd <> -1 Then
                ...
                If EggrafLCbx.SelectedIndex > 0 Then
                    Dim kwdEggrafis As Integer
                    kwdEggrafis = Me.LogariasmoidiamTableAdapter.GetKwdByKratDiamAAEgraf(kwdKratLDB, pelatisKwd, EggrafLCbx.SelectedItem.ToString)
                    Me.LogariasmoidiamTableAdapter.UpdateLogarByKwd(tmimaLKwd, DateTimePicker6.Value, _
                ...
                ElseIf EggrafLCbx.SelectedIndex = 0 Or EggrafLCbx.SelectedIndex = -1 Then
                    ...
                    Me.LogariasmoidiamTableAdapter.InsertLogariasmDiam(kwdKratLDB, tmimaLKwd, DateTimePicker6.Value, _
    ...
    Private Sub remale_kratisi()
        ...
        DateTimePicker2.Value = New DateTime(Me.DbhotelDataSet.kratiseis(0).afixi.Year, Me.DbhotelDataSet.kratiseis(0).afixi.Month, _
        ...
        If isDBEintrag Then     'EDW ALAKSA ->O USER DEN PREPEI GIA NEO EINTRAG NA KSANADWSEI EK NEOY PRAKTOREIO(BLEPE MANIKI->synexomena to idio prakt gia neue Eintraege) 
            Me.SimbolaiaTableAdapter.FillByPraktoreio(Me.DbhotelDataSet.simbolaia, praktoreiokwd, DateTimePicker3.Value.Date.Year)
    ...
    Private Sub BiblPortasBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BiblPortasBtn.Click
        Dim response As MsgBoxResult
        Dim biblioPortas As New BiblioPortasFormclass(imeromErgasias, Me.DbhotelDataSet.etaireia(0).biblportastimes, True, 1)
    ...
    Private Function pruefe_allotment() As Boolean
        ...
        If currentklines <= 0 And currenttipos <= 0 Then 'arxika elgxos ob (0,0) ola me ola->(exw balei >=0 gia na sumperilambanei to -1)
            ...
            Me.AllotmentTableAdapter.FillAllotmBySimbGuarKlinesTiposApoMexri(Me.DbhotelDataSet.allotment, currentsimbolaio, CheckBox1.Checked, DateTimePicker2.Value.Date, DateTimePicker2.Value.Date, 0, 0, _
                                            currentsimbolaio, CheckBox1.Checked, DateTimePicker3.Value.AddDays(-1).Date, DateTimePicker3.Value.AddDays(-1).Date, 0, 0, _
        ...
        Else
            If currenttipos > 0 AndAlso currentklines > 0 Then 'elegxos ob (x,y)
                Me.AllotmentTableAdapter.FillAllotmBySimbGuarKlinesTiposApoMexri(Me.DbhotelDataSet.allotment, currentsimbolaio, CheckBox1.Checked, DateTimePicker2.Value.Date, DateTimePicker2.Value.Date, currentklines, currenttipos, _
                                              currentsimbolaio, CheckBox1.Checked, DateTimePicker3.Value.AddDays(-1).Date, DateTimePicker3.Value.AddDays(-1).Date, currentklines, currenttipos, _
            ...
            If Not allotmOK AndAlso currentklines > 0 Then
                Me.AllotmentTableAdapter.FillAllotmBySimbGuarKlinesTiposApoMexri(Me.DbhotelDataSet.allotment, currentsimbolaio, CheckBox1.Checked, DateTimePicker2.Value.Date, DateTimePicker2.Value.Date, currentklines, 0, _
                                              currentsimbolaio, CheckBox1.Checked, DateTimePicker3.Value.AddDays(-1).Date, DateTimePicker3.Value.AddDays(-1).Date, currentklines, 0, _
            ...
            If Not allotmOK AndAlso currenttipos > 0 Then
                Me.AllotmentTableAdapter.FillAllotmBySimbGuarKlinesTiposApoMexri(Me.DbhotelDataSet.allotment, currentsimbolaio, CheckBox1.Checked, DateTimePicker2.Value.Date, DateTimePicker2.Value.Date, 0, currenttipos, _
                                              currentsimbolaio, CheckBox1.Checked, DateTimePicker3.Value.AddDays(-1).Date, DateTimePicker3.Value.AddDays(-1).Date, 0, currenttipos, _
    ...
    Private Sub TextBox_SelectAllText(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EnilikTbx.Click, PaidiTbx1.Click, PaidiTbx2.Click, _
  EthnAnzTbx1.Click, EthnAnzTbx2.Click, SimetTbx.Click, ProkatTbx.Click, KratisiTbx.Click, PaidiTbx3.Click, EispKataxParSinPlinTbx.Click, EispKataxParEisprTbx.Click, EispKataxParXrewsiTbx.Click, EispKataxParProkTbx.Click
        sender.SelectAll()
    End Sub
    Private Sub timokat_speichern(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If timokatalog.Length <> 0 AndAlso Not DwmTbx.Text.Equals("") Then 'DOMISI->PREPEI NA EXEI KATAXWRITHEI BILA!!!!
            ...
            If kwdikoKratisisDB = -1 Then 'iNSERT PROKEITAI GIA NEo EINTRAG
                ...
                If Not afixiOK And Not overOK Then 'APLH KRATHSH
                    Try
                        ...
                        Else ' DOMISI
                            Try
                                Dim calender As New GoogleCalendar(Me.DbhotelDataSet.etaireia(0).afm, Me.DbhotelDataSet.etaireia(0).doy, maxkrat + 1, OnomTbx1.Text, CType(DateTimePicker2.Value.Date, Date), CType(DateTimePicker3.Value.Date, Date), DwmTbx.Text, EpithimTbx.Text, PraktTbx.Text)
                                calender.send_entry()
                            Catch ex As Exception
                                MsgBox(" Παρουσιάστηκε σφάλμα κατά την Ενημέρωση του Google Calendar !", MsgBoxStyle.Exclamation, "winfo\nikEl.")
                            End Try
                          
                            If Not CType(ProkatTbx.Text, Single) = 0 Then
                ...
                If kwdikos <> -1 Then
                    ...
                    If overOK And status = 1 Then 'edw exw dimiourgisei overunterkuenfte Table-> gia parakolouthisi pou diamenoun ta over-> prepei na anoigei mia maska pou o user na kataxwrei pou paei h kratisi!!!!
                        ...
                        Try
                            ...
                            End Try
                        Catch ex As OleDb.OleDbException ' h kratisi bei over teble ist unique
                            MsgBox("Παρουσιάστηκε πρόβλημα στην καταχώριση των στοιχείων για Over", MsgBoxStyle.Exclamation, "winfo\nikEl.")
                        End Try
                    ...
                    If Not afixiOK And Not overOK Then
                        ...
                        If praktortimolkwd <> 10 Then
                            ...
                            If Me.DbhotelDataSet.links.Count <> 0 Then
                                response = MsgBox(" Ενημέρωση  Leisure?", MsgBoxStyle.YesNo, "winfo\nikEl.")
                                If response = MsgBoxResult.Yes Then
                                    If Not IsDBNull(Me.DbhotelDataSet.links(0).Item("link")) Then
                                        Dim leisure As New Leisure(Me.DbhotelDataSet.links(0).link.ToString, DateTimePicker2.Value.Date)
                                        leisure.Show()
                    ...
                    If countDB = 1 Then ' egine Update ENA eintrag apo Kratiseis
                        ...
                        If count <> 0 Then  'Update Status se periptosi pou yparxei kratisi me dwmatio 
                            ...
                            Else
                                ...
                                If Not Me.DbhotelDataSet.kratiseis(0).dwmatio.Equals(DwmTbx.Text) OrElse (CType(DateTimePicker2.Value.Date, Date)) <> Me.DbhotelDataSet.kratiseis(0).afixi Then
                                    Try
                                        Dim calender As New GoogleCalendar(Me.DbhotelDataSet.etaireia(0).afm, Me.DbhotelDataSet.etaireia(0).doy, "", "", Me.DbhotelDataSet.kratiseis(0).afixi, Me.DbhotelDataSet.kratiseis(0).anaxwrisi, Me.DbhotelDataSet.kratiseis(0).dwmatio, "", "")
                                        calender.del_entry()
                                        calender.set_values(Me.DbhotelDataSet.kratiseis(0).arithmos, OnomTbx1.Text, CType(DateTimePicker2.Value.Date, Date), CType(DateTimePicker3.Value.Date, Date), DwmTbx.Text, EpithimTbx.Text, PraktTbx.Text)
                                        calender.send_entry()
                                    Catch ex As Exception
                                        MsgBox(" Παρουσιάστηκε σφάλμα κατά την Ενημέρωση του Google Calendar !", MsgBoxStyle.Exclamation, "winfo\nikEl.")
                                    End Try
    ...
    Private Sub SendMailtoPraktoreia()
        ...
        Dim dwmadapter As New dbhotelDataSetTableAdapters.dwmatiaTableAdapter
        dwmadapter.Fill(Me.DbhotelDataSet.dwmatia)
        ...
        For j = 0 To Me.DbhotelDataSet.praktoreia.Count - 1
            ...
            Try
                i = Me.DbhotelDataSet.praktoreia(j).email.IndexOf("@") ' einai sosto to mail->dld periexei @??
                If i <> -1 Then
                    ...
                    Else
                        Name = ""
                    End If
                    allbody = body(0) + Name + "," + ChrW(13) + body(1) + "location: " + get_perigrafi_vilas(DwmTbx.Text) + ChrW(13) + "from: " + DateTimePicker2.Value.Date + ChrW(13) + "to: " + DateTimePicker2.Value.Date.AddDays(-1) + ChrW(13) + body(2)

                    'MsgBox("Sending Email to " + mailTemp)

                    SendMail(from, Me.DbhotelDataSet.praktoreia(j).email, subject, allbody, smtpServer, smtpUsername, smtpPassword)
    ...
    End Sub
    Public Function execute_insert_kratisi_afixi_transaction(ByVal arkrat As Int16, ByVal ekptosiOK As Boolean, _
    ByVal countPaid As Int16, ByVal overOK As Boolean, ByVal checkin As Boolean) As Integer
        Using connection As New OleDb.OleDbConnection(connectionString)
            ...
            Try
                ...
                command.Parameters.AddWithValue("imeromkratisis", CType(ImeromKratTbx.Text, Date))
                ...
                command.Parameters.AddWithValue("datein", CType(DateTimePicker2.Value.Date, Date))
                ...
                " klines, tipos, guarantie, dwmatio, pliromi, synolo, anzethnikotites, anzethnikotites2, ethnikotites2, epithimdate, " & _
                "epithimia, prokataboli, prokatdate, simetoxi, tilefonomem, imeromkratisis,simbolaio, ethnikotites, allagesdwmatiwn, checkin, checkout, " & _
                "datein, dateout, ekptosi, ekptposo, ekptpososto, ekptimeres, fee, timologio, identifikation) values (etos, arithmos, praktoreio, " & _
                "praktimologio, voucher, enilikes, paidia, afixi, wra, anaxwrisi, klines, tipos, guarantie, dwmatio, pliromi, synolo, " & _
                "anzethnikotites, anzethnikotites2, ethnikotites2, epithimdate, epithimia, prokataboli, prokatdate, simetoxi, tilefonomem, " & _
                "imeromkratisis,simbolaio, ethnikotites, false, checkin, false ,datein, '1/1/1900', ekptosi, ekptposo, ekptpososto, " & _
                "ekptimeres, fee, 0, identifikation)"
                command.ExecuteNonQuery()
                command.Parameters.Clear()
                ...
                For j = 0 To timokatalog.Length - 1
                    ...
                    command.CommandText = "INSERT INTO timeskratisis (kratisi, timi, ekptosi, apo, mexri, imeres, ypnos, prwino, geuma, deipno, extra, synolo, touristperiod, xrewsi)" & _
                ...
                If checkin Then
                    ...
                    '    ypnosEkpt = (timokatalog(0).ypnos * (1 - ekptPososto / 100)).ToString("N")
                    '    prwinoEkpt = (timokatalog(0).prwino * (1 - ekptPososto / 100)).ToString("N")
                    '    geumataEkpt = ((timokatalog(0).geuma + timokatalog(0).deipno) * (1 - ekptPososto / 100)).ToString("N")
                    '    extasEkpt = (timokatalog(0).paroxi * (1 - ekptPososto / 100)).ToString("N")
                    '    synEkpt = ((timokatalog(0).ypnos + timokatalog(0).prwino + timokatalog(0).deipno + timokatalog(0).geuma + timokatalog(0).paroxi) * (1 - ekptPososto / 100)).ToString("N")
                    '    command.Parameters.AddWithValue("ypnos", ypnosEkpt)
                    '    command.Parameters.AddWithValue("prwino", prwinoEkpt)
                    '    command.Parameters.AddWithValue("geumata", geumataEkpt)
                    '    command.Parameters.AddWithValue("extras", extasEkpt)
                    '    command.Parameters.AddWithValue("synolo", synEkpt)
                    'Else
                    ...
                    'If ekptPososto <> 0 Then
                    '    command.Parameters.AddWithValue("preisemitekpt", True)
                    'Else
                    '    command.Parameters.AddWithValue("preisemitekpt", False)
                    'End If
    ...
    Public Function execute_update_kratisi_afixi_transaction(ByVal connectionString As String, ByVal kwdikos As Integer, ByVal overOK As Boolean, ByVal ekptosiOK As Boolean, ByVal checkIn As Boolean) As Int16
        Using connection As New OleDb.OleDbConnection(connectionString)
            ...
            Try
                ...
                command.CommandText = "UPDATE kratiseis SET praktoreio=?,praktimologio=?, voucher=?, enilikes=?, paidia=?, afixi=?, wra=?, anaxwrisi=?, klines=?, tipos=?, guarantie=?, dwmatio=?," & _
                ...
                If countTimesKrat = timokatalog.Length AndAlso timokatalog(0).kwdDB <> -1 Then
                    For j = 0 To timokatalog.Length - 1
                        ...
                        command.CommandText = "UPDATE timeskratisis SET  timi=?, ekptosi=?, apo=?, mexri=?, ypnos=?, prwino=?, geuma=?," & _
    ...
    Public Function execute_update_diamenwnta_transaction(ByVal connectionString As String, ByVal kwdikos As Integer, ByVal ekptosiOK As Boolean, ByVal overOK As Boolean) As Int16
        Using connection As New OleDb.OleDbConnection(connectionString)
            Dim command, command2, command3 As New OleDb.OleDbCommand()
            Dim transaction As OleDb.OleDbTransaction = Nothing
            Dim rows, j As Int16
            ...
            Try
                ...
                command.Parameters.AddWithValue("praktoreio", CType(kwdikosPrakt, Integer))
                ...
                End If
                command.Parameters.AddWithValue("dwmatio", CType(DwmTbx.Text, String))
                If overOK And Not Me.DbhotelDataSet.kratiseis(0).dwmatio.Equals("Ov-Book") AndAlso imeromErgasias < DateTimePicker3.Value.Date Then ' 
                    DwmTbx.Text = "" 'sto biblio portas tha grafei keno Dwmatio
                    dwmatChangeOK = True
                End If
                command.Parameters.AddWithValue("pliromi", CType(PliromiCbx.SelectedItem, String))
                command.Parameters.AddWithValue("synolo", CType(SynolLbl.Text, Single))
                ...
                command.CommandText = "UPDATE kratiseis SET praktoreio=?,praktimologio=?, voucher=?, enilikes=?, paidia=?, afixi=?, wra=?, anaxwrisi=?, klines=?, tipos=?, guarantie=?, dwmatio=?," & _
                "pliromi=?, synolo=?, ethnikotites=?, anzethnikotites=?, anzethnikotites2=?, ethnikotites2=?, epithimdate=?, epithimia=?," & _
                "prokataboli=?, prokatdate=?, simetoxi=?, tilefonomem=?, simbolaio=?, ekptosi=?, ekptposo=?, ekptpososto=?, ekptimeres=?, fee=? where (kwd=?) and (checkin=true)"
                rows = command.ExecuteNonQuery()
                command.Parameters.Clear()
                    ...
                    For j = 0 To timokatalog.Length - 1
                        If (timokatalog(j).mexri >= imeromErgasias AndAlso imeromErgasias < DateTimePicker3.Value.Date) OrElse praktoreiokwdValue <> -1 Then ' mono oi current periodos (diladi pou eswkleiei tin imeromergasias If status = 2 And praktoreiokwdValue <> -1
                            If timokatalog(j).apo <= imeromErgasias Then
                                ...
                                If timokatalog(j).ypnosDB <> timokatalog(j).ypnosTemp Or timokatalog(j).prwinoDB <> timokatalog(j).prwinoTemp Or _
                                timokatalog(j).deipnoDB <> timokatalog(j).deipnoTemp Or timokatalog(j).geumaDB <> timokatalog(j).geumaTemp Or _
                                timokatalog(j).paroxiDB <> timokatalog(j).paroxiTemp Or timokatalog(j).xrewsiDB <> timokatalog(j).xrewsi Or _
                                (Me.DbhotelDataSet.kratiseis(0).enilikes <> EnilikTbx.Text And Me.DbhotelDataSet.kratiseis(0).afixi.Date = imeromErgasias) Or _
                              (Me.DbhotelDataSet.kratiseis(0).paidia <> countPaid And Me.DbhotelDataSet.kratiseis(0).afixi.Date = imeromErgasias) Then
                                    ...
                                    If myReader.Item("flag") = 1 And myReader.Item("imerominia") = imeromErgasias And _
                                    ...
                                    If myReader.Item("imerominia") <> imeromErgasias Or myReader.Item("flag") = 1 Then
                                        ...
                                        If Not anaxChangeOK Or imeromErgasias <> Me.DateTimePicker3.Value Then
                                            ...
                                            command.Parameters.AddWithValue("datum", CType(DateTimePicker3.Value.Date, Date))
                                            If Me.DwmTbx.Text.Equals("Ov-Book") OrElse ekptosiOK OrElse overOK OrElse Me.DbhotelDataSet.kratiseis(0).anaxwrisi.Date <> Me.DateTimePicker3.Value.Date OrElse CheckBox1.Checked Then
                                            ...
                                            If prodatumOK Then ' progenesteri imerominia Ergasias (apo akyrose kleisimatos imeras)
                                                ...
                                                If anaxChangeOK Then
                                                    ...
                                                    If DateTimePicker3.Value.Date < Me.DbhotelDataSet.kratiseis(0).anaxwrisi.Date Then
                                                        ...
                                                        command2.CommandText = "DELETE FROM biblioportas where (imerominia>=?) and (kratisi=?)"
                                                        command2.ExecuteNonQuery()
                                                        command2.Parameters.Clear()
                                                ...
                                                '    command.Parameters.AddWithValue("imerominia", CType(timokatalog(j - 1).apo, Date))
                                                command2.CommandText = "SELECT * FROM biblioportas where (imerominia>?) and (kratisi=?) and (xrewsi=?) and (ypnos=?) and (prwino=?) and (geumata=?) and (extras=?)  ORDER BY  imerominia ASC, kwd ASC"
                                                ...
                                                While myReader2.Read()
                                                    ...
                                                    'command3.Parameters.AddWithValue("datum", CType(DateTimePicker3.Value, Date))
                                                    If CheckBox1.Checked Then
                                                        If Not paratiriseis.Contains("Guarantee") Then
                                                            paratiriseis = paratiriseis + " Guarantee"
                                                        End If
                                                    ...
                                                    command3.CommandText = "UPDATE biblioportas set xrewsi=?, ypnos=?, prwino=?, geumata=?, extras=?, synolo=?, paratiriseis=? where (kwd=?)" '(imerominia=?) and (status=1) and (kratisi=?)
                                                ...
                                                command2.CommandText = "UPDATE biblioportas set voucher=?, enilikes=?, anilika=?, atoma=?, onomateponimo=?, anzethnikotita=?, ethnikotita=?, anzethnikotita2=?, ethnikotita2=?, praktoreio=? where (kratisi=?)" '(imerominia=?) and (status=1) and (kratisi=?)
                                    ...
                                    Else ' ALLAGH TIMWN TIN IDIA MERA ME AFIXI->UPDATE BIBLIO PORTAS EAN DEN EXEI EKTYPOTHEI
                                        ...
                                        If Not prodatumOK Then 'aktuelle imeromergasias ( ->den uparxei spaetere Eintrag sto Biblio Portas gai tin Kratisi! (apo px akyrosi kleisimatos)
                                            ...
                                            command.Parameters.AddWithValue("synolo", CType((timokatalog(j).ypnos + timokatalog(j).prwino + timokatalog(j).deipno + timokatalog(j).geuma + timokatalog(j).paroxi), Single))
                                            command.Parameters.AddWithValue("datum", CType(DateTimePicker3.Value.Date, Date))
                                            If Me.DwmTbx.Text.Equals("Ov-Book") OrElse ekptosiOK OrElse overOK OrElse CheckBox1.Checked Then 'OrElse Me.DbhotelDataSet.kratiseis(0).anaxwrisi <> Me.DateTimePicker3.Value
                                                If Me.DwmTbx.Text.Equals("Ov-Book") OrElse overOK Then
                                                    If Not paratiriseis.Contains("Ov-Book") Then
                                                        paratiriseis = paratiriseis + " Ov-Book "
                                                    End If
                                                ...
                                                If ekptosiOK Then
                                                    ...
                                                    End If
                                                End If
                                                'If Me.DbhotelDataSet.kratiseis(0).anaxwrisi <> Me.DateTimePicker3.Value Then
                                                '    paratiriseis = paratiriseis + " Αλλαγή Ημερομ. Αναχώρισης"
                                                'End If
                                            End If
                                            If Not prodatumOK Then
                                                command.Parameters.AddWithValue("paratiriseis", paratiriseis) 'myReader.Item("paratiriseis") +
                                            End If

                                            command.Parameters.AddWithValue("kwd", myReader.Item("kwd"))

                                            command.CommandText = "UPDATE biblioportas set voucher=?, dwmatio=?, enilikes=?, anilika=?, atoma=?, xrewsi=?, onomateponimo=?, anzethnikotita=?, ethnikotita=?,anzethnikotita2=?,ethnikotita2=?,praktoreio=?, ypnos=?, prwino=?, geumata=?, extras=?, synolo=?,datum=?, paratiriseis=? where (kwd=?)" '(imerominia=?) and (status=1) and (kratisi=?)
                                            myReader.Close()
                                            rows = command.ExecuteNonQuery()
                                            command.Parameters.Clear()
                                        Else ' edw diadikasia gia allages sto Biblio Portas meta apo akyrosi kleisimatos kai epanafora se 
                                            ...
                                            command2.Parameters.AddWithValue("imerominia", imeromErgasias)
                                            command2.Parameters.AddWithValue("kratisi", CType(kwdikos, Integer))
                                            command2.CommandText = "UPDATE biblioportas set xrewsi=?, ypnos=?, prwino=?, geumata=?, extras=?, synolo=? where (imerominia=?) and (status=1) and (kratisi=?) " '(imerominia=?) and (status=1) and (kratisi=?)
                                  ...
                                  (dwmatChangeOK Or anaxChangeOK Or praktChangeOK Or Me.DbhotelDataSet.kratiseis(0).enilikes <> EnilikTbx.Text Or Me.DbhotelDataSet.kratiseis(0).paidia <> countPaid) Then
                                    ...
                                    If myReader.Item("imerominia") <> imeromErgasias Or myReader.Item("flag") = 1 Then
                                        If Not anaxChangeOK Or imeromErgasias <> Me.DateTimePicker3.Value Then
                                            ...
                                            command.Parameters.AddWithValue("paratiriseis", paratiriseis) ' + paratiriseis
                                            command.CommandText = "INSERT INTO biblioportas (etos, imerominia, status, kratisi, voucher, dwmatio, enilikes, anilika, atoma, xrewsi, onomateponimo, anzethnikotita, ethnikotita, anzethnikotita2, ethnikotita2," & _
                                            "praktoreio, ypnos, prwino, geumata, extras, synolo, datum, paratiriseis, flag) values (etos, imerominia, status, kratisi, voucher, dwmatio, enilikes, anilika, atoma," & _
                                            "xrewsi, onomateponimo, anzethnikotita, ethnikotita, anzethnikotita2, ethnikotita2,praktoreio, ypnos, prwino, geumata, extras, synolo, datum, paratiriseis, 0)"
                                            myReader.Close()
                ...
                If dwmatChangeOK Or anaxChangeOK Then
                    ...
                    If myReader.Item("imerominia") <> imeromErgasias OrElse myReader.Item("flag") = 1 Then 'prepei na knw antigrafi tou telaiytaiou eintrag me anaxwrisi-afixi 
                        ...
                        If Not anaxChangeOK Or imeromErgasias <> Me.DateTimePicker3.Value Then
                            ...
                            command.Parameters.AddWithValue("synolo", CType((timokatalog(j).ypnos + timokatalog(j).prwino + timokatalog(j).deipno + timokatalog(j).geuma + timokatalog(j).paroxi), Single))
                            command.Parameters.AddWithValue("datum", CType(DateTimePicker3.Value.Date, Date))
                            If Me.DwmTbx.Text.Equals("Ov-Book") OrElse overOK OrElse ekptosiOK OrElse CheckBox1.Checked Then 'OrElse anaxChangeOK
                                'paratiriseis = ""
                    ...
                    Else 'idia imerominia->existiert schon Eintrag sto Biblio Portas->nur Update me nneo Dwmatio
                        If Not prodatumOK Then
                            ...
                            command.Parameters.AddWithValue("praktoreio", CType(praktortimolkwd, Integer))
                            command.Parameters.AddWithValue("datum", CType(DateTimePicker3.Value.Date, Date))
                            If Me.DwmTbx.Text.Equals("Ov-Book") OrElse ekptosiOK OrElse overOK OrElse anaxChangeOK OrElse dwmatChangeOK OrElse CheckBox1.Checked Then
                                paratiriseis = ""
                                If dwmatChangeOK Then
                                    If Me.DbhotelDataSet.kratiseis(0).afixi.Date <> imeromErgasias Then
                                        paratiriseis = "αλλ. Δωματίου"
                            ...
                            End If
                            command.Parameters.AddWithValue("paratiriseis", paratiriseis) '
                            'command.Parameters.AddWithValue("paratiriseis", CType(myReader.Item("paratiriseis"), String) + paratiriseis)
                            'command.Parameters.AddWithValue("imerominia", CType(imeromErgasias, Date))

                            'command.Parameters.AddWithValue("kratisi", CType(kwdikos, Integer))
                            command.Parameters.AddWithValue("kwd", myReader.Item("kwd"))
                            command.CommandText = "UPDATE biblioportas set voucher=?, dwmatio=?, praktoreio=?, datum=?, paratiriseis=? where (kwd=?)" '(imerominia=?) and (status=1) and (kratisi=?)
                ...
                If praktChangeOK Then
                    command.Parameters.AddWithValue("praktoreio", CType(praktortimolkwd, Integer))
                    command.Parameters.AddWithValue("kratisi", CType(kwdikos, Integer))
                    command.CommandText = "UPDATE biblioportas set   praktoreio=? where (kratisi=?) " 'imerominia=?) and (status=1) and and 
                    myReader.Close()
                    command.ExecuteNonQuery()
                    command.Parameters.Clear()
                End If
                If onomChangeOK AndAlso Me.DbhotelDataSet.kratiseis(0).afixi.Date = imeromErgasias Then
                    ...
                    End If
                    command.Parameters.AddWithValue("kratisi", CType(kwdikos, Integer))
                    command.CommandText = "UPDATE biblioportas set   onomateponimo=? where (kratisi=?) " 'imerominia=?) and (status=1) and and 
                    myReader.Close()
                    command.ExecuteNonQuery()
                    command.Parameters.Clear()
                End If
    ...
    Private Sub non_show_kratisi(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If kwdikoKratisisDB <> -1 Then
            ...
            If response = MsgBoxResult.Yes Then
                Me.NonshowkratiseisTableAdapter.InsertNonShowKratisi(ImeromKratTbx.Text, etos, Me.DbhotelDataSet.kratiseis(0).arithmos, Me.DbhotelDataSet.kratiseis(0).simbolaio, _
                     praktoreiokwd, praktortimolkwd, Me.DbhotelDataSet.kratiseis(0).privatimologio, VoucherTbx.Text, Me.DbhotelDataSet.kratiseis(0).enilikes, Me.DbhotelDataSet.kratiseis(0).paidia, _
                     Me.DbhotelDataSet.kratiseis(0).ethnikotites, Me.DbhotelDataSet.kratiseis(0).anzethnikotites, Me.DbhotelDataSet.kratiseis(0).ethnikotites2, Me.DbhotelDataSet.kratiseis(0).anzethnikotites2, _
                      Me.DbhotelDataSet.kratiseis(0).afixi.Date, Me.DbhotelDataSet.kratiseis(0).anaxwrisi.Date, currentklines, currenttipos, Me.DbhotelDataSet.kratiseis(0).guarantie, Me.DbhotelDataSet.kratiseis(0).dwmatio, _
    ...
    Private Sub suche_kratiseis(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            End If
        Catch ex As InvalidCastException
            Dim form As New KratiseisSucheF(imeromErgasias, False, False, 1)
    ...
    End Sub
    Private Sub Tbx_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EnilikTbx.Leave, PaidiTbx1.Leave, _
